...
const Recipe = () => {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        const response = await axios.get(`http://localhost:5001/api/recipe/${id}`);
  ...
  const handleSaveRecipe = async (formData) => {
    try {
      await axios.put(`http://localhost:5001/api/recipe/${id}`, formData, {
        headers: {
      ...
      <div className="conteiner">
        <div className="recipe">
          ...
          <div className="recipe-image">
            <img
              src={recipe.img ? `http://localhost:5001/static/${recipe.img}` : Foto}
              alt={recipe.name}
