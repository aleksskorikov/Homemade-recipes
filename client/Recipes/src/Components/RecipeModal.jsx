...
import '../styles/scss/_RecipeModal.scss';
            ...
            <div className="modal-content">
                ...
                <form onSubmit={handleSubmit}>
                    <label className='modal__form-lable'> Введіть назву рецепту:
                        <input type="text" placeholder="Введите название рецепта" value={name} onChange={(e) => setName(e.target.value)} className='modal__form-input'/>
                    </label >
                    ...
                    <label className='modal__form-lable'> Блок інгредієнтів 1 (введіть назву):
                        <input type="text" placeholder="Введите название блока 1" value={block1Title} onChange={(e) => setBlock1Title(e.target.value)} className='modal__form-input'/>
                        {ingredientsBlock1.map((list, index) => (
                            <input key={`block1-${index}`} type="text" placeholder={`Ингредиент ${index + 1}`} value={list} onChange={(e) => handleChangeList(1, index, e.target.value)} className='modal__form-input'/>
                        ))}
                    ...
                    <div>
                        <label className='modal__form-lable'>Блок інгредієнтів 2 (введіть назву):
                            <input type="text" placeholder="Введите название блока 2" value={block2Title} onChange={(e) => setBlock2Title(e.target.value)} className='modal__form-input'/>
                            {ingredientsBlock2.map((list, index) => (
                                <input key={`block2-${index}`} type="text" placeholder={`Ингредиент ${index + 1}`} value={list} onChange={(e) => handleChangeList(2, index, e.target.value)} className='modal__form-input'/>
                            ))}
                    ...
                    <label className='modal__form-lable'> Опис:
                        <textarea placeholder="Введите описание" value={description} onChange={(e) => setDescription(e.target.value)} className='modal__form-textarea'/>
                    </label>
